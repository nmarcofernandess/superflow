...
def test_release_metadata() -> None:
    ...
    if package.get("name") != PACKAGE_NAME or package.get("version") != "0.11.0":
        raise AssertionError("package.json não declara @superflow/runtime 0.11.0")
    ...
    for relative_path in (
        ...
        if manifest.get("name") != "superflow" or manifest.get("version") != "0.11.0":
            raise AssertionError("{} não está na versão 0.11.0".format(relative_path))
    ...
    for marketplace, source in (
        ...
        entry = entries[0]
        if entry.get("name") != "superflow" or entry.get("version") != "0.11.0":
            raise AssertionError("marketplace não está na versão 0.11.0")
