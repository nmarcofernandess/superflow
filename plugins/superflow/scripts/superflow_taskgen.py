...
def build_values(args: argparse.Namespace, classification: dict) -> dict:
    ...
    return {
        ...
        # WARLOG campaign shell (warlog-contract.md)
        "mission": (
            f"Deliver “{title}” as a durable Superflow campaign with mergeable sprints."
        ),
        "scope_in": "The described request and its mergeable slices.",
        "scope_out": "Unrelated epics; microtask tracking (use Plan/log).",
        "first_sprint_title": f"First mergeable slice toward {title}",
        "sprint_budget": (
            "direct"
            if classification["phase_budget"] in {"capture", "lean"}
            else "spec"
            if classification["phase_budget"] in {"deep", "forensic"}
            else "plan"
        ),
    }
